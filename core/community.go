...
func CreateCommunity(ctx context.Context, db *sql.DB, creator uid.ID, reqPoints, maxPerUser int, name, about string) (*Community, error) {
	...
	if err := IsUsernameValid(name); err != nil {
		return nil, httperr.NewBadRequest("invalid-community-name", fmt.Sprintf("Неверное название сообщества. Оно %s.", err.Error()))
	}
	...
	if communityCreationAdminOnly {
		...
		if user.Points < reqPoints {
			return nil, httperr.NewForbidden("not-enough-points", "У вас недостаточно прав, чтобы создать сообщество.")
		}
		...
		if n >= maxPerUser {
			return nil, httperr.NewForbidden("max-limit-reached", "На данный момент вы достигли максимального количества сообществ, которые можете создать.")
		}
	}
	...
	if exists, _, err := CommunityExists(ctx, db, name); err != nil {
		...
		return nil, &httperr.Error{HTTPStatus: http.StatusConflict, Code: "community-exists", Message: fmt.Sprintf("Сообщество с именем %s уже существует.", name)}
	}
...
func GetCommunities(ctx context.Context, db *sql.DB, sort CommunitiesSort, set string, n int, viewer *uid.ID) ([]*Community, error) {
	if !slices.Contains([]string{CommunitiesSetAll, CommunitiesSetDefault, CommunitiesSetSubscribed}, set) {
		return nil, httperr.NewBadRequest("invalid-set", "Недопустимые параметры набора сообщества.")
	}
	...
	default:
		return nil, httperr.NewBadRequest("invalid-sort", "Недопустимый параметр сортировки по сообществу.")
	}
...
func MakeUserMod(ctx context.Context, db *sql.DB, c *Community, viewer uid.ID, user uid.ID, isMod bool) error {
	...
	if is, err := c.UserMod(ctx, viewer); err != nil {
		...
		if !actionUser.Admin {
			return httperr.NewForbidden("not-mod-not-admin", "Пользователь не является ни модератором, ни администратором.")
		}
	}
	...
	if !addMod && !actionUser.Admin {
		...
		if !higher && !(actionUser.ID == user) {
			return httperr.NewForbidden("lower-mod", "Пользователь находится ниже в иерархии модов.")
		}
	}
...
func scanCommunityRules(db *sql.DB, rows *sql.Rows) ([]*CommunityRule, error) {
	...
	if len(rules) == 0 {
		return nil, httperr.NewNotFound("rules-not-found", "Правила сообщества не найдены.")
	}
	return rules, nil
}
