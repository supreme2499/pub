...
var (
	ErrInvalidFeedCursor = httperr.NewBadRequest("invalid_cursor", "Недействительный курсор пагинации ленты.")
	ErrInvalidFeedSort   = httperr.NewBadRequest("invalid-sort", "Недействительная сортировка ленты.")
)
...
func GetUserFeed(ctx context.Context, db *sql.DB, viewer *uid.ID, userID uid.ID, filter string, limit int, next *uid.ID) (*UserFeedResultSet, error) {
	if !(filter == "posts" || filter == "comments" || filter == "") {
		return nil, httperr.NewBadRequest("invalid-filter", "Фильтр должен быть одним из 'постов' или 'комментариев', или должен быть пустым.")
	}
