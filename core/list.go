...
func (o ListItemsSort) MarshalText() ([]byte, error) {
	text := o.String()
	if text == "" {
		return nil, httperr.NewBadRequest("invalid-list-sort", "Недопустимая сортировка списка.")
	}
...
func (o *ListItemsSort) UnmarshalText(data []byte) error {
	...
	return httperr.NewBadRequest("invalid-list-sort", "Недопустимая сортировка списка.")
}
...
func GetList(ctx context.Context, db *sql.DB, id int) (*List, error) {
	...
	if len(lists) == 0 {
		return nil, httperr.NewNotFound("list-not-found", "Список не найден.")
	}
	return lists[0], nil
}
...
func GetUsersLists(ctx context.Context, db *sql.DB, user uid.ID, sort, filter string) ([]*List, error) {
	...
	if !(sort == "name" || sort == "lastAdded") {
		return nil, httperr.NewBadRequest("invalid-lists-sort", "Недопустимая сортировка списков.")
	}
	if !(filter == "all" || filter == "public" || filter == "private") {
		return nil, httperr.NewBadRequest("invalid-lists-filter", "Неверный фильтр списков")
	}
...
func listnameValid(name string) error {
	if err := IsUsernameValid(name); err != nil {
		return httperr.NewBadRequest("invalid-list-name", fmt.Sprintf("Название списка %v", err))
	}
	return nil
}
