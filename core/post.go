...
func validatePost(title, body string) error {
	if len(title) < 3 {
		return httperr.NewBadRequest("post/title-too-short", "Название слишком короткое.")
	}
	return nil
}
...
func createPost(ctx context.Context, db *sql.DB, opts *createPostOpts) (*Post, error) {
	...
	if community.PostingRestricted {
		if is, err := community.UserModOrAdmin(ctx, opts.author); err != nil {
			...
			return nil, httperr.NewForbidden("posting-restricted", "Публикации в этом сообществе ограничены.")
		}
	}
...
func CreateLinkPost(ctx context.Context, db *sql.DB, author, community uid.ID, title string, link string) (*Post, error) {
	errInvalidURL := httperr.NewBadRequest("invalid-url", "Неверный URL-адрес.")
...
func (p *Post) Delete(ctx context.Context, user uid.ID, g UserGroup, deleteContent bool, sendNotif bool) error {
	...
	if deleteContent && !(g == UserGroupNormal || g == UserGroupAdmins) {
		return httperr.NewForbidden("mod-cannot-delete-content", "Модераторы не могут удалять содержимое публикации. Они могут только удалить публикацию из своего сообщества.")
	}
...
func (p *Post) Pin(ctx context.Context, user uid.ID, siteWide, unpin bool, skipPermissions bool) error {
	if p.Deleted && !unpin {
		return httperr.NewForbidden("cannot-pin-deleted-post", "Не удается закрепить удаленные записи.")
	}
	...
	return msql.Transact(ctx, p.db, func(tx *sql.Tx) (err error) {
		...
		if unpin {
			...
			if reached, err := maxPinsReached(ctx, tx, community); err != nil {
				...
				return httperr.NewForbidden("limit-reached", "Достигнут максимальный предел закрепленных сообщений.")
			}
		}
	...
	if err != nil {
		...
		if msql.IsErrDuplicateErr(err) {
			return &httperr.Error{
				HTTPStatus: http.StatusConflict,
				Code:       "already-voted",
				Message:    "Пользователь уже проголосовал.",
			}
		}
		return err
	}
