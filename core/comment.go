...
func addComment(ctx context.Context, db *sql.DB, post *Post, author *User, parentID *uid.ID, commentBody string) (*Comment, error) {
	...
	if parentID != nil {
		...
		if parent.Deleted {
			return nil, httperr.NewBadRequest("comment-reply-to-deleted", "Не удается ответить на удаленный комментарий.")
		}
		if parent.Depth == maxCommentDepth {
			return nil, httperr.NewBadRequest("comment-max-depth-reached", "Невозможно ответить, потому что достигнута максимальная глубина вложенности.")
		}
	...
	err := msql.Transact(ctx, c.db, func(tx *sql.Tx) error {
		if _, err := tx.ExecContext(ctx, "INSERT INTO comment_votes (comment_id, user_id, up) VALUES (?, ?, ?)", c.ID, user, up); err != nil {
			if msql.IsErrDuplicateErr(err) {
				return httperr.NewBadRequest("already-voted", "Вы уже проголосовали за этот комментарий.")
			}
			return err
		}
